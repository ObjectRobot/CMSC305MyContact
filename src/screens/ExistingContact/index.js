...
const ExistingContact = props => {
    ...
    const onContactUpdate = () => {
        if (!fullname){
            alert('Please enter a contact full name.');
            return;
        }
        if (!phone){
        ...
        myContactsDB.transaction(txn => {
            txn.executeSql(
                `UPDATE ${contactsTableName} SET fullname = '${fullname}', phone = '${phone}' WHERE id = ${post.id}`,
                [],
                () => {
                    console.log(`${fullname} ${phone} updated successfully.`);
                },
                error => {
  ...
  return (
    <View style={styles.container}>
        ...
        <View style={styles.bottomContainer}>
            ...
            <Pressable style={styles.addButton} onPress={onAddContact}>
